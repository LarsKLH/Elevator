...
In order to interact with the elevator we have two threads as part of the elevator interface module. The elevator output periodically takes the current state from memory and tries to mirror the physical elevsator with regards to that state. The elevator inputs listens to any inputs from the elevators and forwards them to memory. 
